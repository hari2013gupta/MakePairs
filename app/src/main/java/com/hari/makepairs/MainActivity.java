...
public class MainActivity extends Activity implements OnClickListener{
	...
	int secc=0;boolean game_bool=false;
	long millis_cdt=0;
...
//        Linkify.addLinks(url, Linkify.WEB_URLS);
       ...
       buttonListener = new ButtonListener();
		...
		if (extras != null){	
			...
			if (diff.equalsIgnoreCase("diff1")) {nx = 3;ny = 4;TotalTime=TotalTime/2;}else if (diff.equalsIgnoreCase("diff2")) {nx = 4;ny = 4;TotalTime=TotalTime/2+TotalTime/4;}else if (diff.equalsIgnoreCase("diff3")) {nx = 4;ny = 5;}else if (diff.equalsIgnoreCase("diff4")) {nx = 5;ny = 6;TotalTime=TotalTime+TotalTime/2;}else if (diff.equalsIgnoreCase("diff5")) {nx = 6;ny = 6;TotalTime=TotalTime*2;
			}else if (diff.equalsIgnoreCase("diff6")) {nx = 6;ny = 7;TotalTime=TotalTime*2;}else if (diff.equalsIgnoreCase("diff7")) {nx = 5;ny = 5;}else if (diff.equalsIgnoreCase("diff8")) {nx = 5;ny = 6;}else if (diff.equalsIgnoreCase("diff9")) {nx = 6;ny = 7;}else if (diff.equalsIgnoreCase("diff10")) {nx = 7;ny = 7;
			}}newGame(nx,ny);
...
//			mTxtField.setTypeface(font);mTxtField.setText("");
			mTxtField.setTextColor(Color.GREEN);mTxtField.setTextSize(30);
...
//    					mp_failed.start();
    					cdt.pause();
    					if(bool_alert){
    						AlertCustomGameWin();bool_alert=false;
    					}
    				}
    				else Count++;
    			}
...
//	    dismissButton.setBackgroundResource(R.drawable.btn_resume);
	    _buttonNextReplay.setOnClickListener(new OnClickListener() {
	        @Override
	        public void onClick(View v) {
	        	game_bool=true;cdt.cancel();game_bool=false;
...
protected void onDestroy() {
	Audio.release();mp_failed.release();mp_win.release();
	super.onDestroy();
}
...
protected void GameWinActivity(){
	Intent i=new Intent(MainActivity.this,GameWin.class);
	i.putExtra("level", diff);i.putExtra("bool_sound", bool_sound);
